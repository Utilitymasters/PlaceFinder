package com.place.finder.component.database;
...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    // create new record
    // @param recentHistoryItem contains details to be added as single row.
    public boolean create(RecentHistoryItem recentHistoryItem) {

        boolean createSuccessful = false;

        if(!checkIfExists(recentHistoryItem.itemName)){

            SQLiteDatabase db = this.getWritableDatabase();

            ContentValues values = new ContentValues();
            values.put(fieldObjectName, recentHistoryItem.itemName);
            createSuccessful = db.insert(tableName, null, values) > 0;

            db.close();

            if(createSuccessful){
                Log.e(TAG, recentHistoryItem.itemName + " created.");
            }
        }
    ...
    public List<RecentHistoryItem> getAll() {

        List<RecentHistoryItem> recordsList = new ArrayList<RecentHistoryItem>();
       ...
       /* sql += " WHERE " + fieldObjectName + " LIKE '%" + searchTerm + "%'";
        ...
        if (cursor.moveToFirst()) {
            do {
                ...
                RecentHistoryItem recentHistoryItem = new RecentHistoryItem(objectName);

                // add to list
                recordsList.add(recentHistoryItem);
